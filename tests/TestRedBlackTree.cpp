...
constexpr uint64_t elements = 1024*1024*40;
constexpr uint64_t cc = 1024*1024;
...
struct NodeAccessor {
	inline static uint64_t Color(TreeAccessor* tree, void* node) {
		return ((Node*)node)->Color<FIELD2>();
	}
	inline static void Color(TreeAccessor* tree, void* node, uint64_t newColor) {
		((Node*)node)->Color<FIELD2>(newColor);
	}
	inline static void* Left(TreeAccessor* tree, void* node) {
		return ((Node*)node)->Left<FIELD2>(tree->origin);
	}
	inline static void Left(TreeAccessor* tree, void* node, void* newLeft) {
		((Node*)node)->Left<FIELD2>(tree->origin, (Node*)newLeft);
	}
	inline static void* Right(TreeAccessor* tree, void* node) {
		return ((Node*)node)->Right<FIELD2>(tree->origin);
	}
	inline static void Right(TreeAccessor* tree, void* node, void* newRight) {
		((Node*)node)->Right<FIELD2>(tree->origin, (Node*)newRight);
	}
	inline static void* Parent(TreeAccessor* tree, void* node) {
		return ((Node*)node)->Parent<FIELD2>(tree->origin);
	}
	inline static void Parent(TreeAccessor* tree, void* node, void* newParent) {
		((Node*)node)->Parent<FIELD2>(tree->origin, (Node*)newParent);
	}
	inline static uint64_t Value(TreeAccessor* tree, void* node) {
		return ((Node*)node)->GetValue<FIELD2>();
	}
};

uint64_t CompareTrees(
	RedBlackTreeAllocator& rbtree,
	Generic::RedBlackTree<TreeAccessor, NodeAccessor>& grbt,
	std::multiset<Node*>& stdset) {
	
	uint64_t invalid = 0;
	
	Node* rbtreeIt = rbtree.FindGreaterEqual<FIELD>(0);
	auto stdIt = stdset.begin();
	auto grbtIt = grbt.begin();
	
	for(;;) {
		void *a=(void*)3, *b=(void*)5, *c=(void*)23;
		a = rbtreeIt;
		if(grbtIt != grbt.end())
			b = *grbtIt;
		if(stdIt != stdset.end())
		...
		if(a != c || a != b || b != c) {
			printf(" invalid values found: rbtree: %p  grbt: %p  stdset: %p\n", a, b, c);
			++invalid;
		}
		
		uint64_t ends = 0;
		if(rbtreeIt) {
			rbtreeIt = rbtreeIt->Next<FIELD>(nodes);
			if(rbtreeIt == NULL)
				++ends;
		} else
			ends++;
	...
	printf(" CompareTrees: invalid comparisons with std::set found: %llu\n", invalid);
...
// 	std::multimap<Node*, Node*> test_m;
	...
	*/
	...
	if(false)
	{
		RedBlackTreeAllocator *rbtrees = new RedBlackTreeAllocator[cc];
		TreeAccessor* accessors = new TreeAccessor[cc];
		Generic::RedBlackTree<TreeAccessor, NodeAccessor>* grbts = new decltype(grbt)[cc];
		for(int i=0; i<cc; ++i) {
			rbtrees[i].SetOrigin(nodes);
			grbts[i].tree = accessors+i;
			accessors[i].origin = nodes;
		}
		if(true) {
			float times[32];
			uint64_t counts[32];
		...
		if(true) {
			...
			for(int J=0; J<32; ++J) {
				float time = times[J];
				I = counts[J];
				printf(" grbt small trees (%i) inserting: %lu nodes in %.2fs -> %.3f kops\n", J+1, I, time, I/(time*1000.0f));
			}
		}
		DEBUG;
		if(true) {
			...
			Start();
			uint64_t I=0;
			std::multiset<Node*>* sets = new std::multiset<Node*>[cc];
			for(int J=0; J<32; ++J) {
				for(uint64_t i=0; i<cc; ++i) {
					for(uint64_t j=0; j<1; ++j, ++I) {
						sets[i].insert(all[I]);
			...
			for(int J=0; J<32; ++J) {
				float time = times[J];
				I = counts[J];
				printf(" std::set small trees (%i) inserting: %lu nodes in %.2fs -> %.3f kops\n", J+1, I, time, I/(time*1000.0f));
			}
			delete[] sets;
		}
		delete[] accessors;
	...
	for(int j=0; j<elements/cc; ++j) {
		...
		Start();
		for(uint64_t i=0; i<cc; ++i) {
			rbtree.RBTInsert<FIELD>(all[used-cc+i]);
		}
		End();
		time = DeltaTime();
		...
		*/
		...
		CompareTrees(rbtree, grbt, test_s);
...
// 		rbtree.Root<FIELD>()->Print<FIELD>(nodes);
		grbt.Root()->VerifyParenting(false, ttt);
		...
		*/
		...
		
		{
			...
			for(;;) {
				if(rbtreeIt) {
					...
					break;
				sum += (uint64_t)rbtreeIt;
			}
			End();
			time = DeltaTime();
			printf(" rbtree iterated %lu nodes in %.2fs -> %.2f kops\n", test_s.size(), time, test_s.size()/(time*1000.0f));
			...
			End();
			time = DeltaTime();
			printf(" grbt iterated %lu nodes in %.2fs -> %.2f kops\n", test_s.size(), time, test_s.size()/(time*1000.0f));

			auto* grbtIt2 = grbt.begin().node;
			Start();
			for(;;) {
				if(grbtIt2) {
					grbtIt2 = grbtIt2->Next(ttt);
					if(grbtIt2 == NULL)
						break;
				} else
					break;
				sum += (uint64_t)grbtIt2;
			}
			End();
			time = DeltaTime();
			printf(" grbt->Next() iterated %lu nodes in %.2fs -> %.2f kops\n", test_s.size(), time, test_s.size()/(time*1000.0f));
			...
			End();
			time = DeltaTime();
			printf(" std::set iterated %lu nodes in %.2fs -> %.2f kops\n", test_s.size(), time, test_s.size()/(time*1000.0f));
			printf(" sum = %llu\n", sum);
		}
		...
		{
			printf("\n");
			auto* it = grbt.FindGreaterEqual(0);
			for(; it; it = it->Next(ttt)) {
				printf("  %lu", ((size_t)it-(size_t)nodes)/64);
			}
			printf("\n");
			it = grbt.FindGreaterEqual(0);
			uint64_t prev = 0;
			for(; it; it = it->Next(ttt)) {
				if(prev > it->Value(ttt)) {
					printf(" %lu > %lu ", prev, it->Value(ttt));
				}
				prev = it->Value(ttt);
			}
			printf("\n");
		...
		uint64_t size = used;
		printf(" rbtree elments: %.2fM -> height: %lu, minHeight: %lu\n", size/1000000.0f, height, minHeight);
		
		height = grbt.Root()->Height(ttt);
		minHeight = grbt.Root()->MinHeight(ttt);
		printf(" grbt elments: %.2fM -> height: %lu, minHeight: %lu\n", size/1000000.0f, height, minHeight);
	}
